...
internal class ServerChat {
	...
	internal unsafe ServerChat(ISigScanner scanner) {
		if (scanner.TryScanText(Signatures.SendChat, out nint processChatBoxPtr)) {
			...
			Moon.Log?.Information("Found signature for chat sending");
		}
		else {
            Moon.Log?.Error("Can't find signature for chat sending, functionality will be unavailable!");
		}
		...
		if (scanner.TryScanText(Signatures.SanitiseString, out nint sanitisePtr)) {
            ...
            Moon.Log?.Information("Found signature for chat sanitisation");
		}
		else {
            Moon.Log?.Error("Can't find signature for chat sanitisation, functionality will be unavailable!");
		}
	}
    ...
    public void SendChat(string chatline)
    {
        ...
        string cleaned = SanitiseText(chatline);
